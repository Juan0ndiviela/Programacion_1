...
int main() {
    ...
    for (double grados = 0; grados <= 180; grados += 10) {
        ...
        cout << fixed << setprecision(0) << setw(ANCHO) << grados << fixed << setprecision(DECIMALES) << setw(ANCHO) << radianes << fixed << setprecision(DECIMALES) << setw(ANCHO) << sin(radianes) <<fixed << setprecision(DECIMALES) <<  setw(ANCHO) << cos(radianes) << endl;
    }
}

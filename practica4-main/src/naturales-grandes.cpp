...
void escribirEnPantalla(const unsigned natural[]){
    for(unsigned i = 0; i < numCifras(natural); i++){
        cout << natural[i];
    }
}
...
void calcularImagen(const unsigned natural[], unsigned imagen[]) {
    ...
    for(unsigned i = 0; i < cifras; i++){
        imagen[i] = natural[cifras - i - 1];
    }
    for(int j = cifras + 1; j < NUM_DIGITOS; j++){
        imagen[j] = 0;
    }
}
